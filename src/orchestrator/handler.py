...
def _resolve_dao(team: str = "", version: str = "") -> DbDao:
    if team and version:
...
def _run_team_pipeline(team: str, version: str, request_obj: Dict[str, Any]) -> Dict[str, Any]:
    ...
    dao.put_run_meta(run_id=run_id, status="RUNNING", payload={"owner": owner, "request": request_obj})
    ...
    while step_index < len(workflow):
        ...
        if not agent:
            dao.put_run_meta(run_id=run_id, status="FAILED", payload={"owner": owner, "error": f"Agent not found for step {step_id}"})
        ...
        try:
            ...
            dao.put_run_meta(run_id=run_id, status="FAILED", payload={"owner": owner, "error": f"JSON parse failed on {step_id}: {e}"})
        ...
        if not schema:
            dao.put_run_meta(run_id=run_id, status="FAILED", payload={"owner": owner, "error": f"Schema not found for ref {schema_ref}"})
        ...
        try:
            ...
            dao.put_run_meta(run_id=run_id, status="FAILED", payload={"owner": owner, "error": f"Schema validation failed on {step_id}: {msg}"})
    ...
    dao.put_run_meta(run_id=run_id, status="SUCCEEDED", payload={"owner": owner, "steps": list(outputs.keys())})
...
def handler(event, context):
    ...
    if p == "/improve/tasks" and m == "GET":
        ...
        limit = _safe_int(qs.get("limit") or 50, 50)
        dao = _resolve_dao(team=qs.get("team") or "", version=qs.get("version") or "")
    ...
    if p == "/improve/task/done" and m == "POST":
        ...
        dao = _resolve_dao(team=body.get("team") or "", version=body.get("version") or "")
